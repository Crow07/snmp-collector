...
metric_file = open('../config/switch_cfg.yaml') ##路径写全路径或者相对路径
metric = yaml.load(metric_file)

info_file = "/var/log/collector/check_switch_info.log"

metric_collection = metric['NetworkMetric']
...
def snmpMethod(method,session,metric,var_list):
    if method == 'GET':
	...
	if i['value'] == 1: 
	     ...
	     for kv in metric_collection:
			 ...
			 metric = n['key']
                         ...
                         snmpMethod(method,session,metric,var_list)
             while True:
                 try:
